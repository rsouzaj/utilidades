...
# Questionario de sono Pittsburgh -----------------------------------------
...
## (qualidade subjetiva do sono) = pittsburg_q16 (Como você classificaria sua qualidade do
...
# Componente 2 (latência do sono) = soma de (escore  pittsburg_q2) com
#                                   pittsburg_q5( não conseguiu dormir até 30 min)
...
# Componente 3 - duração do sono - comp3 ----------------------------------

# componente 3 (duração do sono em horas de sono)= pittsburg_q4
# tempo      escore
# >7           0
# 6-7          1
# 5-6          2
...
# Componente 4 - eficiência do sono - comp4 -------------------------------

## Relação entre as horas de sono e o tempo que se passa na cama
...
pitt_table <- pitt_table %>% ## opção usando o pacote lubridate(), fica mais elegante
  mutate(
    pittsburgh_q1   = lubridate::as_datetime(pittsburgh_q1),
    pittsburgh_q3   = lubridate::as_datetime(pittsburgh_q3),
    overnight       = pittsburgh_q3 < pittsburgh_q1, ## cria um função boleana (TRUE vs FALSE).
    ...
    comp4           = (pittsburgh_q4/comp4)*100, ### porcentagem da eficiência do sono.
    comp4           = case_when(
      comp4 >   85 ~ 0,
      comp4 >=  75 & comp4 <= 84 ~ 1,
      comp4 >=  65 & comp4 < 75 ~ 2,
...
### Mulheres muito "eficientes" no sono: acima de 100% de eficiência - efi_100
# pitt_table %>%
#   mutate(
#     pittsburgh_q1   = lubridate::as_datetime(pittsburgh_q1),
#     pittsburgh_q3   = lubridate::as_datetime(pittsburgh_q3),
#     overnight       = pittsburgh_q3 < pittsburgh_q1, ## cria um função boleana.
#
#     pittsburgh_q3   = pittsburgh_q3 + lubridate::hours(overnight*24),
#     ## se overnight = TRUE, acrescenta 24 hs.
...
# Componente 5 - distúrbios do sono - comp5 -------------------------------
...
# Componente 6 - uso de medicação - comp6 ---------------------------------

pitt_table <- mutate(pitt_table, comp6 = pittsburgh_q17)



# Componente 7 - função diurna - comp7 ------------------------------------
...
# Hmisc::label(pitt_table$comp1_cat) = "Subjetive sleep quality"
# Tabela final ------------------------------------------------------------
